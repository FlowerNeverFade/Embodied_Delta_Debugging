...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "risk_critic_large_eval"
...
EXPECTED_CAUSAL_SCHEMA = "shed-cfs-causal-v3-multimodal"
...
def _report_path(report_dir: Path, task_id: int, init_state_id: int, seed: int) -> Path:
    return report_dir / f"task{task_id:02d}_init{init_state_id:02d}_seed{seed:02d}_causal_v3.json"
...
def _case_command(args: argparse.Namespace, report_path: Path, task_id: int, init_state_id: int, seed: int) -> list[str]:
    cmd = [
        str(LIBERO_PYTHON),
        str(PROJECT_ROOT / "pi05_natural_failure_probe.py"),
        "--policy-host",
...
def _start_policy_server(args: argparse.Namespace, output_dir: Path) -> Optional[subprocess.Popen]:
    ...
    if args.policy_server_kind == "openpi":
        ...
        cmd = [
            str(LEROBOT_PYTHON),
            str(PROJECT_ROOT / "serve_lerobot_pi0fast_policy.py"),
            "--port",
            str(args.policy_port),
            "--policy-dir",
            str(args.policy_dir),
