...
for i in "${!GPU_IDS[@]}"; do
  ...
  PILOT_OUTPUT_DIR="$shard_output" POLICY_PORT="$port" "$RUN_DIR/status_pilot_video_eval.sh" || true
  echo
done
...
print(
    "invalid_init="
    f"{sum(1 for r in rows if r.get('status') == 'invalid_init' or r.get('semantic_status') == 'invalid_init')}"
)
